...
**Data:** 8328 UFC fights 2003-02-28 to 2026-09-12,
102 features computed strictly as-of fight date by replaying career
...
(1678 fights). LightGBM (best (31, 0.03, 60), cv 0.6547) and XGBoost
(best (4, 0.03, 2, 0.6, 0.8, 1), cv 0.6556) tuned with 4-fold expanding-window CV inside
...
(LGB, XGB, logistic): [0.32, 0.43, 0.11].

## Model comparison — full test set (1678 fights)

| Predictor | n | Accuracy | Log loss | Brier | ECE |
|---|---|---|---|---|---|
| Stacked ensemble (LGB+XGB+logistic) | 1678 | 0.636 | 0.642 | 0.225 | 0.022 |
| Ensemble + isotonic | 1678 | 0.636 | 0.644 | 0.226 | 0.031 |
| LightGBM | 1678 | 0.631 | 0.644 | 0.226 | 0.030 |
| XGBoost | 1678 | 0.629 | 0.643 | 0.226 | 0.030 |
| Logistic regression (full features) | 1678 | 0.613 | 0.649 | 0.229 | 0.052 |
| Elo only (logistic) | 1678 | 0.566 | 0.676 | 0.241 | 0.045 |

## The real benchmark — vig-removed closing odds (1182 test fights with odds)

| Predictor | n | Accuracy | Log loss | Brier | ECE |
|---|---|---|---|---|---|
| Market (no-vig closing odds) | 1182 | 0.702 | 0.582 | 0.199 | 0.034 |
| Stacked ensemble | 1182 | 0.652 | 0.630 | 0.220 | 0.040 |
| Blend: market + model | 1182 | 0.704 | 0.577 | 0.197 | 0.023 | logit blend, model coef=+0.293
...
10,000 resamples): 95% CI [+0.0010, +0.0096], blend better in
99.1% of resamples.
...
|---|---|---|
| 6-way outcome log loss | 1.598 | 1.714 (freq) — per-class isotonic REJECTED (2.065) |
| 6-way LL, method-odds subset (n=1105) — model / blend w=0.9 | 1.531 | 1.584 (model alone) |
| 6-way outcome top-1 accuracy | 0.346 | 0.240 |
| Goes the distance — Brier (dedicated model) | 0.241 | 0.234 (6-way-derived) |
| Goes the distance — accuracy | 0.593 | 0.501 (majority class) |
| Under 2.5 rounds (3R fights) — Brier | 0.238 | 0.248 (predict base rate) |
| Fighter sig over 24.5 — Brier (duration-aware) | 0.219 | 0.232 (base rate) |
| Fighter td over 0.5 — Brier (duration-aware) | 0.218 | 0.245 (base rate) |
| Total takedowns — MAE | 1.67 | 1.82 (train mean) |
...
ECE (10 equal-count bins): model 0.040, market 0.034.
...
- `age_diff`: +0.0298
- `red_corner`: +0.0236
- `form_sapm_diff`: +0.0026
- `elo_decline_diff`: +0.0023
- `n_fights_diff`: +0.0020
- `td_avg_diff`: +0.0020
- `elo_diff`: +0.0019
- `sapm_diff`: +0.0018
- `str_def_diff`: +0.0014
- `sc_share_diff`: +0.0012
...
- Odds are closing odds from the Ultimate UFC Dataset (1182/1678
