...
**Data:** 8316 UFC fights 2003-02-28 to 2026-09-05,
...
(1666 fights). LightGBM (best (31, 0.06, 20), cv 0.6547) and XGBoost
(best (4, 0.03, 5, 0.8, 0.6, 5), cv 0.6560) tuned with 4-fold expanding-window CV inside
...
(LGB, XGB, logistic): [0.63, 0.14, 0.17].

## Model comparison — full test set (1666 fights)

| Predictor | n | Accuracy | Log loss | Brier | ECE |
|---|---|---|---|---|---|
| Stacked ensemble (LGB+XGB+logistic) | 1666 | 0.621 | 0.644 | 0.227 | 0.025 |
| Ensemble + isotonic | 1666 | 0.623 | 0.652 | 0.229 | 0.031 |
| LightGBM | 1666 | 0.628 | 0.648 | 0.228 | 0.028 |
| XGBoost | 1666 | 0.630 | 0.644 | 0.226 | 0.038 |
| Logistic regression (full features) | 1666 | 0.612 | 0.649 | 0.229 | 0.051 |
| Elo only (logistic) | 1666 | 0.567 | 0.675 | 0.241 | 0.046 |
...
| Stacked ensemble | 1182 | 0.632 | 0.633 | 0.221 | 0.037 |
| Blend: market + model | 1182 | 0.704 | 0.577 | 0.197 | 0.025 | logit blend, model coef=+0.303
...
10,000 resamples): 95% CI [+0.0006, +0.0093], blend better in
98.7% of resamples.
...
|---|---|---|
| 6-way outcome log loss | 1.602 | 1.715 (freq) — per-class isotonic REJECTED (2.211) |
| 6-way LL, method-odds subset (n=1105) — model / blend w=0.8 | 1.527 | 1.588 (model alone) |
| 6-way outcome top-1 accuracy | 0.332 | 0.239 |
| Goes the distance — Brier (dedicated model) | 0.239 | 0.235 (6-way-derived) |
| Goes the distance — accuracy | 0.597 | 0.503 (majority class) |
| Under 2.5 rounds (3R fights) — Brier | 0.237 | 0.249 (predict base rate) |
...
| Total takedowns — MAE | 1.68 | 1.82 (train mean) |
...
ECE (10 equal-count bins): model 0.037, market 0.033.
...
- `age_diff`: +0.0325
- `red_corner`: +0.0302
- `form_sapm_diff`: +0.0027
- `elo_decline_diff`: +0.0027
- `n_fights_diff`: +0.0026
- `elo_diff`: +0.0022
- `str_def_diff`: +0.0018
- `sapm_diff`: +0.0018
- `sc_share_diff`: +0.0016
- `form_was_finished_diff`: +0.0012
...
- Odds are closing odds from the Ultimate UFC Dataset (1182/1666
