...
**Data:** 8276 UFC fights 2003-02-28 to 2026-08-15,
...
(1626 fights). LightGBM (best (31, 0.03, 20), cv 0.6527) and XGBoost
...
(LGB, XGB, logistic): [0.42, 0.28, 0.17].

## Model comparison — full test set (1626 fights)

| Predictor | n | Accuracy | Log loss | Brier | ECE |
|---|---|---|---|---|---|
| Stacked ensemble (LGB+XGB+logistic) | 1626 | 0.632 | 0.640 | 0.224 | 0.032 |
| Ensemble + isotonic | 1626 | 0.632 | 0.643 | 0.226 | 0.030 |
| LightGBM | 1626 | 0.635 | 0.641 | 0.224 | 0.033 |
| XGBoost | 1626 | 0.633 | 0.641 | 0.225 | 0.033 |
| Logistic regression (full features) | 1626 | 0.614 | 0.648 | 0.229 | 0.053 |
| Elo only (logistic) | 1626 | 0.568 | 0.675 | 0.241 | 0.050 |

## The real benchmark — vig-removed closing odds (1182 test fights with odds)

| Predictor | n | Accuracy | Log loss | Brier | ECE |
|---|---|---|---|---|---|
| Market (no-vig closing odds) | 1182 | 0.702 | 0.582 | 0.199 | 0.031 |
| Stacked ensemble | 1182 | 0.644 | 0.630 | 0.220 | 0.038 |
| Blend: market + model | 1182 | 0.704 | 0.577 | 0.197 | 0.025 | logit blend, model coef=+0.287
...
10,000 resamples): 95% CI [+0.0011, +0.0095], blend better in
99.4% of resamples.
...
|---|---|---|
| 6-way outcome log loss | 1.608 | 1.714 (freq) — per-class isotonic REJECTED (2.337) |
| 6-way LL, method-odds subset (n=1105) — model / blend w=0.8 | 1.526 | 1.594 (model alone) |
| 6-way outcome top-1 accuracy | 0.333 | 0.242 |
| Goes the distance — Brier (dedicated model) | 0.239 | 0.236 (6-way-derived) |
| Goes the distance — accuracy | 0.611 | 0.502 (majority class) |
...
ECE (10 equal-count bins): model 0.038, market 0.031.
...
- `red_corner`: +0.0229
- `elo_diff`: +0.0037
- `n_fights_diff`: +0.0025
- `td_avg_diff`: +0.0022
- `elo_decline_diff`: +0.0020
- `form_sapm_diff`: +0.0020
- `sapm_diff`: +0.0014
- `form_was_finished_diff`: +0.0013
- `str_def_diff`: +0.0012
...
- Odds are closing odds from the Ultimate UFC Dataset (1182/1626
